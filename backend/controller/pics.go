...
func UpdatePics(c *gin.Context) {
  userid := c.MustGet("userID").(int16)
     ...
     if err != nil {
         ...
         if !validateFileType(file) || file.Size > maxFileSize {
             c.JSON(http.StatusBadRequest, gin.H{
                 "error": "Nieprawidłowy plik lub przekroczony rozmiar",
             })
             return
         }

         // Usuń stary plik i zapisz nowy
